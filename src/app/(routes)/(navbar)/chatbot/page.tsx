...
const [reportParams, setReportParams] = useState<ReportParamsType>({sign: {emotioanalAppeal: {evidence: [], chances: 0}, monetaryAppeal: {evidence: [], chances: 0}, urgencyAppeal: {evidence: [], chances: 0}, unsolicitedAppeal: {evidence: [], chances: 0}, sensitiveInformation: {evidence: [], chances: 0}}, validation: {timestamp: {evidence: [], chances: 0}, number: {evidence: [], chances: 0}, email: {evidence: [], chances: 0}, location: {evidence: [], chances: 0}}, summary:""});
...
const sendText = async (userInput : string) => {
  ...
  const res = await fetch("/api/openai", {
    ...
    body: JSON.stringify({
      messages: [...messages, { role: "user", content: " You are an AI scam detector called VerifyAI, base on all the chat messages, help me answer this:" + userInput }],
      model: "gpt-4o",
      temperature: 0.7,
    }),
  });
...
const handleToggleShowReport = async () => {
  if (reportParams.summary === "") {
    ...
    const data = await res.json();
    const content = data.response.content.replace(/```json\n?/, "").replace(/```/, "");
    console.log(content);
  ...
  return (
    <div className={styles.container}>
      {showReport ? <Report params={reportParams} toggleShowReport={handleToggleShowReport}/>
      : <>
        <HistoryTab historyChats={historyChats}/>
        <div className={styles.chat_container}>
          <ChatMessageBox messages={messages} />
          <UserInputContainer onSendMessage={sendText} toggleShowReport={handleToggleShowReport}/>
        </div>
      </>}
    </div>
  );
